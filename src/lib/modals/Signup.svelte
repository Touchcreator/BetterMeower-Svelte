        ...
        {:else}
            <form
                on:submit|preventDefault={e => {
                    ...
                    doLogin();
                    console.log("Account sucessfully created via signup modal")
                    return false;
                }}
            >
                {#if loginStatus}
                ...
                {/if}
                <input type="text" class="modal-input white" placeholder="Username" maxlength="20" value={username} autofocus><br /><br />
                <input type="password" class="modal-input white" placeholder="Password" maxlength="64" value={password}><br />
                <p class="checkboxes">
                ...
                <div class="modal-buttons">
                    <a href="/" on:click|preventDefault={()=>{modalPage.set("login")}}>Login to Meower</a>
                    <button type="submit" disabled={!acceptTerms}>Create Account</button>
                </div>
            </form>
        {/if}
	</div>
